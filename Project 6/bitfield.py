...
class BitField(object):
    ...
    def __init__(self, from_bit: int, to_bit: int):
        ...
        the low-order 4 bits could be represented by from_bit=0, to_bit=3.
        
        Args:
            from_bit: the lower-order bit at the start position of the region to extract
            to_bit: the higher-order bit at the end position of the region to extract
        """
    ...
    def extract(self, word: int) -> int:
        ...
        For example, if we are extracting bits 3..5, the result will be an
        integer between 0 and 7 (0b000 to 0b111).
        
        Args:
            word: the word (collection of bits) to extract from
        Returns:
            The extracted bitfield segment of size (self.to_bit - self.from_bit + 1)
        """
    ...
    def insert(self, value: int, word: int) -> int:
        """Insert value, which should be in the low order
         ...
         Args:
            value: the bitfield to insert into the low order bits of the bitfield
            word: the word to insert into (size must be >= size of value)
        Returns:
            The combined value after the insertion
        Example: 
    ...
    def extract_signed(self, word: int) -> int:
        ...
        Returns:
            The signed integer corresponding to the bitfield
        """
...
def sign_extend(field: int, width: int) -> int:
    """Interpret the field as a signed integer with width bits.
    If the sign bit is zero, it is positive.  If the sign bit
    is negative, the result is sign-extended to be a negative
    integer in Python.
    The width must be 2 or greater. field must fit in width bits.
    
    Args:
        field: the field to interpret as a signed integer
        width: the width of the field
    Returns:
        The signed integer corresponding to the bitfield field
    Examples:
        Suppose we have a 3 bit field, and the field
        value is 0b111 (7 decimal).  Since the high
        bit is 1, we should interpret it as
